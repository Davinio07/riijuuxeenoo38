...
| `/elections/{electionId}/national` | --- | `GET` | URL param `{ electionId }` | **200:** `{ nationalResults: [{ partyName, votes, percentage }] }`<br>**204:** *No Content* (no results available)<br>**404:** `{ error: "Election not found" }`<br>**500:** `{ error: "Internal server error" }` | Fetches the national election results (e.g. votes per party) for the given election ID. |
| `/elections/{electionId}/seats` | --- | `GET` | URL param `{ electionId }` | **200:** `{ seats: { [partyName]: number } }`<br>**204:** *No Content* (no seat data available)<br>**404:** `{ error: "Election not found" }`<br>**500:** `{ error: "Internal server error" }` | Retrieves the number of seats per party for the given election ID at the national level. |
